...
from ...models import Main, Vuz, Regions, Districts, Ministries, Program, Training
...
class Command(BaseCommand):
    ...
    def load_regions(self, file_path):
        ...
        for _, row in df.iterrows():
            ...
            )  # Предполагается, что id_district у вас есть в Excel
    ...
    def load_training(self, file_path):
        ...
        for _, row in df.iterrows():
            program = Program.objects.get(
                progid=row["progid"]
            )  # Предполагается, что progid у вас есть в Excel
        ...
        for _, row in df.iterrows():
            try:
                ...
                vuz_instance = Vuz.objects.get(
                    id_listedu=row["id_listedu"], id_parent=row["id_parent"]
                )
                # Получаем информацию для других полей, если нужно
                Prog = Program.objects.get(
                    progid=row["progid"]
                )  # Измените на соответствующее название столбца
                ...
                self.stderr.write(
                    self.style.ERROR(f"Error loading Main: {e} for row: {row}")
                )
